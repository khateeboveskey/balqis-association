        ...
        <div class="hidden w-full md:block md:w-auto" id="navbar-default">
          <ul class="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-white">
            <li v-for="(route, index) in routes" :key="index">
              <NuxtLink active-class="font-bold text-primary-800" :to="route.path" class="block py-2 px-3 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-primary-800 md:p-0">{{ route.title }}</NuxtLink>
            </li>
          </ul>
        </div>
      </div>
